...
export function buildProjectsMindmap(projects, notes) {
  return projects.map(project => ({
    ...
    icon: 'rocket',
    children: (project.assigned_users || []).map(user => ({
      id: `person-${user}`,
      title: user,
      icon: 'users',
      children: notes
        .filter(note => note.project === project.name && note.assigned_to === user)
        .map(task => ({
          id: `task-${task.id}`,
          title: task.title,
  ...
  return uniquePeople.map(person => ({
    ...
    children: projects
      ...
      .map(project => ({
        ...
        icon: 'rocket',
        children: notes
          .filter(note => note.project === project.name && note.assigned_to === person)
          .map(task => ({
            id: `task-${task.id}`,
            title: task.title,
            description: task.content,
